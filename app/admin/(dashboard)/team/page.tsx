import { getAllTeamMembers, getPageContent, text } from '@/lib/cms/queries'
import { AdminCard, Field, TextArea, TextInput } from '@/components/admin/ui'
import { SubmitButton } from '@/components/admin/submit-button'
import { ImageField } from '@/components/admin/image-field'
import type { TeamGroupKey, TeamMember } from '@/lib/cms/types'
import {
  addTeamMemberAction,
  ...
  return (
    <div className="flex gap-6 rounded-md border border-border p-4">
      <ImageField
        ...
        aspect="aspect-[4/5]"
      />
      <form action={updateTeamMemberAction.bind(null, member.id)} className="flex flex-1 flex-col gap-3">
...
export default async function TeamAdminPage() {
  const [content, members] = await Promise.all([getPageContent('team'), getAllTeamMembers()])

  return (
    <div className="flex flex-col gap-6">
      <div>
        <h1 className="text-2xl font-semibold text-foreground">Team</h1>
        <p className="mt-1 text-sm text-muted-foreground">Manage photos, roles, and bios for every team member.</p>
      </div>

      <form action={updateTeamTextAction}>
        <AdminCard title="Team page copy">
          <div className="grid grid-cols-2 gap-4">
            ...
            <Field label="Intro body" htmlFor="intro_body"><TextArea id="intro_body" name="intro_body" rows={2} defaultValue={text(content, 'intro_body')} className="col-span-2" /></Field>
        ...
        return (
          <AdminCard key={group.key} title={group.label}>
            ...
            <details className="mt-4">
              <summary className="cursor-pointer text-sm font-medium text-primary">+ Add a team member</summary>
              <form action={addTeamMemberAction.bind(null, group.key)} className="mt-3 flex flex-col gap-3">
                <div className="grid grid-cols-2 gap-3">

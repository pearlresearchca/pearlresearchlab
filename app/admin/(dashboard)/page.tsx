...
const sections: { href: string; label: string; description: string; icon: LucideIcon; section: SectionKey }[] = [
  ...
  { href: '/admin/research', label: 'Research areas', description: 'The research streams shown on Home and the Research page.', icon: FlaskConical, section: 'research' },
...
export default async function AdminDashboard() {
  ...
  let recent: Pick<AuditLogEntry, 'id' | 'table_name' | 'action' | 'changed_by_email' | 'changed_at' | 'old_data' | 'new_data'>[] = []
  if (isAdmin) {
    const insforge = await createInsForgeServerClient()
    const { data } = await insforge.database
      .from('content_audit_log')
      .select('id, table_name, action, changed_by_email, changed_at, old_data, new_data')
              ...
              {recent.map((entry) => (
                <li key={entry.id} className="flex items-center gap-3 py-2.5 text-sm">
                  ...
                  <span className="flex-1 text-foreground">
                    ...
                    {auditSummary(entry.new_data ?? entry.old_data) && <span className="text-muted-foreground"> — {auditSummary(entry.new_data ?? entry.old_data)}</span>}
                  </span>
